...
if not game.CoreGui:FindFirstChild("StreamSniper") then
    ...
    TBox1.Position = UDim2.new(1, 0, 0.3, 0)
    TBox1.Text = ""
    ...
    TBox3.PlaceholderText = "Type TP to Verify"
    ...
    if _G.cursor == nil then
        ...
        _G.cursor = http:JSONDecode(_G.cursor.Body)
    end
        ...
        spawn(
            function()
                ...
                function checktokens(tokens)
                    ...
                    local s, data = pcall(http.JSONDecode, http, result.Body)
                    return data.data
                end
                    ...
                    local suc, err =
                        ...
                        function()
                            if _G.cursor[tostring(gameid)] and _G.available then
                                ...
                                _G.image = http:JSONDecode(url.Body).data[1].imageUrl
                                ...
                                for i, v in ipairs(a) do
                                    spawn(
                                        function()
                                            ...
                                            a[i] = http:JSONDecode(a[i].Body).data
                                            if a[i] then
                                                if not a[i][100] then
                                                    b = true
                                                    num = i
